...
partitions of the domain as:

\be\label{bilinear}
w_{i}(t,\mathbf{s}) :=(\frac{t_{i}-t_{i-1}}{t_{N}-t_{0}}\right)
(t-t_{0}),\mathbf{a}_i+\mathbf{b}_i\frac{t_{i}-t_{i-1}}{t_{N}-t_{0}}+\mathbf{c}_i\mathbf{s}+\mathbf{d}_i+\frac{t_{i}-t_{i-1}}{t_{N}-t_{0}}\right)
(t-t_{0})\mathbf{s}),
\ee

\noindent $t$ is time and $\mathbf{s}$ is a point in chord symmetry space $\{P, I, T, v, a\}$, and
so the parameters $\mathbf{a, b, c, d}$ are vector-valued. In
...
\subsection{Parametric Maps for Fractal Approximations}

In order to chart a parametric map over a range of fractal approximations, a
simple procedure may be used. First, an order is defined for the parameters on
the RHS of \eqref{bilinear}: simply $\{t, P, I, T, v, a\}$. Of course, two
different FAs may have different numbers of mappings in their respective
Hutchinson operators. We account for that by padding the shorter operator with
zeros. Then, we acknowledge that the actual ordering is not significant as long
as the different sets of parameters are laid out in the \textit{same} order.
Here, that is ${t_i, s_i, y_i}$ for each mapping in the operator. We take the
smaller of two parameters $W_{min}$ as the upper left corner of the map, and the
larger of the two parameters $W_{max}$ as the lower right corner of the map. If
we define a fixed number of interpolants between the corners, we may then use
simple linear interpolation to locate any parameter between those corners. In
other words, we divide up the map between the corners by $N$ intervals for the
two most significant parameters. We then divide each square in the same way for
the next two most significant parameters, and so on. The score graph is then
computed for each of the points in the final map, the actual score is rendered
from the score graph, and the parameter point is colored by some quantitative
feature of the score. Alternatively, a thumbnail visual rendering of the score,
a condensed piano roll, may be used; that is our approach here.

Then the $u_i$ in \eqref{IfsForFif} are implied by $t$ at each level of
recursion, and the $v_i$ are implied by the $s_i$ and $y_i$ at each level of
recursion.



The data values must of course have a one-to-one correspondence with the index
values. This is done by sorting the data values and ensuring that there are at
least as many Hilbert values as data values.

In the literature, the transformations used to construct Hutchinson operators
for FIFs may take several forms. In the simplest form, they are simple
