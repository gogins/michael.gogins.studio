...
(IFSs) music, for using IFS to generate music in spaces with harmonic structure.
We define a score as the \textit{graph} of a function that maps time to a
...
more detail; discusses the implementation of IFSs, indexes for parameters, and
...
domain of the bounded mapping, and contracts the range of the mapping.
...
the FIF scheme. For the most part, these reflect the original intention of
interpolating functions to fit existing data points. But that is not our
intention here. For the purposes of adapting this method for musical
composition, we adopt from \cite{2012arXiv1209.3139B} one of the simpler and
clearer transformations, the bilinear transformation, expressed with linear
partitions of the domain and a vector-valued range as:

\be\label{bilinear}
w_{i}(t,\mathbf{s}) :=[t_{i-1} + \frac{t_{i}-t_{i-1}}{t_{N}-t_{0}}\right)
(t-t_{0}),\mathbf{a}_i+\mathbf{b}_it+\mathbf{c}_i\mathbf{s}+\mathbf{d}_it
\mathbf{s}],
\ee

\noindent $t$ is time and $\mathbf{s}$ is a point in chord symmetry space $\{P,
I, T, v, a\}$, and so the parameters $\mathbf{a, b, c, d}$ are vector-valued. In
\cite{2012arXiv1209.3139B} additional constraints are added that cause the
interpolated sub-domains and ranges to fit neatly together as in Figure
\ref{fig:fif} but, as we note, these are not used here.
...
\eqref{bilinear}: simply $\{t, \mathbf{a, b, c, d}\}$. Similarly, within each
Hutchinson operator, we order the transformations by the order of their
...
operators. To account for that, we pad the shorter operator with zeros.  We take
the smaller of two parameters $W_{min}$ as the upper left corner of the map, and
the larger of the two parameters $W_{max}$ as the lower right corner of the map.
If we define a fixed number of parameter points between the corners, we may then
use simple linear interpolation to calculate any parameter point between those
corners. To do this, we divide up the map between the corners by $N$ intervals
for the two most significant parameters. We then divide each square in the same
way for the next two most significant parameters, and so on. The score graph is
then computed for each of the points in the final map, the actual score is
rendered from the score graph, and the parameter point is colored by some
quantitative feature of the score. Alternatively, a thumbnail visual rendering
of the score, a condensed piano roll, may be used; that is our approach here.
...
The following pseudo-code generates a score from the virtual parameter point of an IFS
for a score graph.

\begin{lstlisting}
// From the upper and lower corners of the parametric map, the number of points
// on the height and width of the map, and the maximum number of transformations
// in a corner, derive the strides for each parameter.

// Starting from the lower corner of the map, sum the strides on each real
// parameter until the virtual parameter point has been reached, to derive the
// final values of all parameters in the actual Hutchinson operator.

// Recursively apply the Hutchinson operator to compute the FA for the score
// graph. Recursion should stop when the target depth of recursion has been
// reached. At that level of recursion, translate the point on the score graph
// into a chord consisting of one or more notes with the appropriate octavewise
// voicing and arrangement of voices, and add the notes of that chord to the
// score. Skip transformations that are not local.

var hutchinson = function(maximum_depth, current_depth, operator, t_min, t_max, point, score) {
    current_depth = current_depth + 1;
    if (current_depth >= maximum_depth) {
        score.append(point_to_notes(time, point));
        return;
    }
    var first_x = 0;
    var final_x = operator[operator.length - 1].t;
    for (var transformation_index = 0; transformation_index < operator.length; transformation_index++) {
        var transformation = operator[transformation_index];
        var current_x = transformation.t;
        // Partition the domain.
        var current_t = point.t;
        var next_t = prior_x + ((current_x - prior_x) / (final_x - first_x)) * (current_t - first_x);
        // Apply the transformation.
        if (transformation.local_to(transformation_index) {
            var new_t = point.t
            hutchinson(maximum_depth, current_depth, operator, local_t_min, local_t_max, point, score);
        }
    }

}

// Quantize pitches to MIDI key numbers and voices to instrument numbers.

// Join overlapping notes in the score.

// Display a piano roll view of the score and play it using MIDI, Csound, etc.
\end{lstlisting}
