...
musical score in some score space. Finally, we define a \textit{parametric score
generator} as one whose behavior is completely defined by numerical parameters.

\textit{Parametric composition} is then the art of composing music by exploring
...
$OPTIC$), inversion $I$, transposition $T$, octavewise revoicings $V$,
permutations of instruments for voices $A$, permutations of dynamic levels for
voices $L$, and permutations of note durations for voices $D$. In this space, a
piece of note-based music may be considered a succession of more or less
fleeting chord points or, in other words, as the graph of a vector-valued
function of time mapped to chord symmetry space, which we call a \textit{score
graph} of a \textit{score function},

\be \label{score_graph}
G(f: x \mapsto P, I, T, V, A, L, D),
\ee
...
particularly, our score generators are special IFS whose contractive
transformations are constrained such that the IFS computes
attractors closely approximating the graphs of functions. The function for
...
the genetic algorithm for IFS parameters in the CsoundAC library 
\cite{csoundac} for algorithmic composition in C++; and finishes with 
examples of each of the three methods of algorithmic composition, implemented 
in CsoundAC and Csound \cite{csoundmain}.
...
original publications, but also to recent reviews or summaries.
...
All musicians are aware that pitches transposed to different octaves
...
equivalence}. Octave equivalence transforms the line of pitches into a circle, an
...
assist the understanding, pitches are rounded by semitone). This is $OPTC$, which is
what musicians normally mean by ``chord type.''

\begin{figure}
\centerline{\includegraphics[width = 0.75\textwidth]{OPTTC}}
...
commonly used meaning of what is a ``chord.''

\begin{figure}
\centerline{\includegraphics[width = 0.75\textwidth]{OPTTIC}}
...
\item $I$ -- Inversion, \emph{i.e.}\ reflection in the inversion flat of the chord space.
\item $T$ -- Transposition within octave equivalence.
\end{enumerate}
...
% B(X,Y) @>\Phi>>  B(X,Y)
% \end{CD}
% The commutativity of the diagram (2.13) then holds with F F replaced by F and F
% replaced by F F .
% \ee
...
I, T, V, A, L, D\}$, and $\mathbf{s}$ are vector-valued scaling factors that ensure
...
planes by time, using a modification of Equation \eqref{bilinear2} that
...
\subsection{Composing Sound Directly}
...
\subsection{Analysis and Re-Composition}
