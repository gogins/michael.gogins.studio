...
(IFSs) music, for using IFSs to generate music in spaces with harmonic
structure. We define a score as the \textit{graph} of a function that maps time
to a succession of more or less fleeting chords, which are represented as points
in a discrete space of chord symmetries. Such score graphs can be computed using
...
score graphs $G(f)$. The advantage of such IFSs is that (in principle)
...
interpolating a given set of data points.
...
\be\label{bilinear}
w_{i}(t,\mathbf{s}) :=[t_{i-1} + \frac{t_{i}-t_{i-1}}{t_{N}-t_{0}})
...
\cite{2012arXiv1209.3139B} the interpolated sub-domains and ranges are neatly
fitted together (Figure \ref{fig:fif}) using Equation \eqref{bilinear2}, where
$x$ is the time dimension of the score graph, $y$ is the chord symmetry
dimension of the score graph, the $X$ are interpolation points for time, $Y$ are
vector-valued interpolation points for $\{P, I, T, V, A\}$, and $s$ are
vector-valued scaling factors that ensure contraction to the fixed point of the
attractor, i.e.\ the score graph, and provide some control over the shape of the
graph in between the interpolation points. Note that $N$ interpolation points
define $N-1$ transformations in the Hutchinson operator.

\be\label{bilinear2}
\begin{align}
w_{n} &  (x,y)= \left(X_{n-1}+ \left(  \frac{X_{n}-X_{n-1}}{X_{N}-X_{0}}\right)
(x-X_{0}),Y_{n-1}+\left(  \frac{Y_{n}-Y_{n-1}}{X_{N}-X_{0}%
}\right)  (x-X_{0})\right.\label{w}\\
& \left. + \left[  s_{n-1}+\left(  \frac{s_{n}-s_{n-1}}{X_{N}-X_{0}}\right)
(x-X_{0})\right]  \left[  y-Y_{0}-\left(  \frac{Y_{N}-Y_{0}}{X_{N}-X_{0}%
...
\eqref{bilinear2}: simply $\{X, Y, s\}$. Similarly, within each Hutchinson
operator, we order the transformations by the order of their parameters. The
actual ordering is not significant as long as the different sets of parameters
are laid out in the \textit{same} order. Of course, two different FAs may have
different numbers of mappings in their respective Hutchinson operators. To
account for that, we pad the shorter operator with zeros.
