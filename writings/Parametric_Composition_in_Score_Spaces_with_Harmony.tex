...
matrices in the time-harmony subspace of the Hutchinson operator from $N$
interpolation points \eqref{interpolationPoints} as follows.  The shear
transformations and scalings are constrained such that $F_i(I_0) = I_{i-1}$ and
$F_i(I_N) = I_i$ for every $i = 1, 2, \dots, N$. Solving these constraints
...
and harmony. These transformations can be used, \emph[e.g.}\, to add different
pitches to a chord.
