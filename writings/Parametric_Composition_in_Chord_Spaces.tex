...
spaces with harmonic structure, extending our earlier work on generating scores
with iterated function systems (IFSs). We define a score as the \textit{graph}
of a score function that maps time to a succession of more or less fleeting
chords, which are represented as points in a discrete space of chord symmetries.
Such score graphs can be computed using special iterated function systems (IFS)
that compute the graphs of fractal interpolation functions (FIFs), which are
related to the Read-Bajraktarevi\'c operator. The Collage Theorem proves that
any such graph may be approximated, as closely as desired, by such a IFS. One
method of composing is then to manually construct these IFSs, or to evolve them
using the genetic algorithm, and then to render the IFS to an actual score.
Additionally, using Hilbert indexes, a single complex virtual parameter can be
...
or to interpolate between different scores located in such a map.
\end{abstract}
...
Here, we extend our previous work by generating scores in a special score space
...
\noindent Notes and chords do
\textit{not} need to be confined to 12-tone equal temperament. One motivation
for using this score space to compose is that different values on the chord
dimensions of the space are clearly audible as harmonic differences. Another
motivation is that, at any one point in time, there is one and only one
``harmony.''
...
\noindent We supplement this chord space by adding two more dimensions:
...
continuous. The purpose of chord space is to permit composing by moving a single
point through the space and thus controlling at once set class, inversion,
transposition, voicing, and arrangement; another way of saying this is that any
piece of note-based music can be represented by the graph of a vector-valued
function of time (Equation \eqref{score_graph}).

By assigning each symmetry to an orthogonal dimension of chord space, the
...
One basic method of generating fractals is with the multiple copy reduction
...
represents an affine transformation of the plane. Each lens, chosen at random or
in turns, transforms the original set, and the union of the transformations
...
a \textit{Read-Bajraktarevi\'c} operator $\Phi$. The literature on fractal
interpolation is extensive, and mainly concerned with interpolating a given set
of data points; here, we do not use this capability, but rather focus on the use
of IFSs to compose functions from scratch.
