...
Hilbert indices work because all metric spaces have the same cardinality; therefore, there is always a one-to-one mapping between points in any $N$-dimensional space and points on the line or, more usefully, the plane.
...
A major result of this research been the elucidation of a provable hierarchy of complexity classes for problems that are solvable by computer \parencite{arora2009computational}. Other important results are proofs there are completely abstract machines that are not incomplete; these are called super-Turing and depend, one way or another, on doing arithmetic with real numbers that are usually not Turing computable because they never terminate. 
 ...
 For our purposes, the important complexity classes are as follows.
...
\begin{description}
\item[ST]  As noted above, some of the Julia sets that might be encountered in computing a parametric map are, in fact, super-Turing.
\item[RE] Most likely all Julia sets that would be of musical interested can be approximated as closely as desired -- are recursively enumerable.
\item[NP] It is also very likely that many of these Julia sets are not computable in polynomial time. If so, then computing a parametric map that is big enough to be useful for composers might remain forever computationally intractable.
\item[P] But if P = NP, then in principle it is possible to produce a useful parametric map in polynomial time. If so, then parametric composition would open up a whole new world for composers.
\end{description}

If P = NP, then a high-resolution parametric map of considerable subsets of compositions would, as it were, display symmetries and patterns, such as those found in the Mandelbrot set, that could assist in parametric composition. It would be like having a God's-eye view of possible structure in music, and would partly overcome the irreducibility of algorithmic composition. In other words, P = NP could imply that \emph{intelligibility }can be achieved in algorithmic composition.

But if P != NP, it would in practice not be possible to produce a high-resolution parametric map of any considerable subset of compositions. And this would fracture and obstruct the intelligibility of algorithmic composition.

I have now added to my list of mathematical things with fundamental implications for algorithmic composition:

\begin{description}
\item[Universality] Not only is it indeed possible to write a computer program that can generate any finite score as precisely as one likes, but it is often also possible to greatly compress the amount of information required to represent the score. This, of course, is one of the fundamental motivations for pursuing algorithmic composition.
...
\item[Intelligibility] For the time being, intractability implies that any high-level, comprehensive intelligibility of algorithmic composition is out of reach. Yet, if P = NP, such maps could be made, and could provide an intelligible window into heretofore obscure symmetries and patterns in musical structure.
\end{description}
