...
Use a consistent toolkit on both macOS and Linux. This should be as small as possible. New code should not be written, except as actual pieces, or to expand CsoundAC or cloud-5 for the express purpose of writing new pieces.
 ...
 - For Bandcamp, the upload should be a high-resolution FLAC soundfile formatted as stereo, 24 bit depth, 96000 Hz. Fill in Bandcamp's metadata fields, as the original metadata may be omitted or overwritten. If the master file is rendered by a cloud-5 piece, produce the FLAC soundfile and cover image using `playpen.py post-processing <filename>`. The license should be Attribution-NonCommercial-ShareAlike 4.0 International.