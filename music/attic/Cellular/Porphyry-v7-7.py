...
TODO: Find out why silent gaps occur -- dead measures?
        ...
        for minuet_column in range(column_begin, columns_to_play + column_begin):
            for minuet_row in range(row_begin, rows_to_play + row_begin):
                ...
                if (scale_count > 1 and time_offset == 0):
                    ...
                    modulation_count_modulus = modulation_count % modulations_per_combination
                    if modulation_count_modulus == 0:
                        combination_count = combination_count + 1
                        combination = (combination_count % 4) + 1
                        sequence_holder.getScore().append(cumulative_time, 1., 144., 15., 1., combination, combination)
                ...
                for k in range(repetitions_for_measure):
                    ...
                    print("Loaded '%s' at %9.4f:\n%s" % (filename, real_time, score_for_measure.toString()))
                    ...
                    print("score_duration: %9.4f" % (score_duration))
