...
extern "C" int score_generator(CSOUND *csound) {
    ...
    generators.push_back([&chordsForTimes, &modality, &base_level](const Cursor &pen_, int depth, int target_depth, csound::Score &score) {
        Cursor pen = pen_;
    ...
    for (int i = 0, n = score.size(); i < n; ++i) {
        ...
        auto duration = score[i].getDuration() / 60.;
