...
The score of this piece is generated in the actual Csound file using my 
csound-cxx-opcodes, which compile C++ source code embedded in the file to a 
...
the piano sound, which is synthesized by Modartt's Pianoteq plugin, 
a complete physical model of a concert grand, accessed via my 
csound-vst3-opcodes.

The score is displayed in a Web page (also embedded in the piece) using my 
csound-webserver-opcodes as a three-dimensional piano roll, along with 
controls for adjusting the levels and parameters of the instruments.
...
// Unlike original instrument, give this one a definite stereo pan.

a_signal = (a_out_left + a_out_right) / 2.;
a_out_left, a_out_right pan2 a_signal, k_space_left_to_right
...
gk_PianoOutPianoteq_level init -1.3
gi_FMWaterBell_attack init 0.002936276551436901
gi_FMWaterBell_release init 0.022698875468554768
gi_FMWaterBell_exponent init 0
gi_FMWaterBell_sustain init 5.385256143273636
...
///gk_Sweeper_level init -23
gk_Sweeper_level init -28
gk_Buzzer_harmonics init 4
gk_Buzzer_level init 12
gk_Shiner_level init 15
gk_Blower_grainDensity init 132.3332789825534
...
extern "C" int score_generator(CSOUND *csound) {
    ...
    auto key = score[index].getKey() + 5;
    score[index].setKey(key);
    auto duration = score[index].getDuration() * 3.;
    score[index].setDuration(duration);
