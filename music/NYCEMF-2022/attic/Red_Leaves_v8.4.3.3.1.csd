...
Ambisonic spatializer originally written by Jan Jacob Hoffman and re-written by 
me, with Doppler effects and many distance cues. The plugin Csound opcodes used 
in this piece are available on GitHub at https://github.com/gogins, except for 
the vst4cs opcodes which can be downloaded from 
...
gk_FMWaterBell_left_to_right init 5
...
<body style="background-color:black;box-sizing:border-box;padding:10px;:fullscreen">
    ...
    <script>
        ...
        window.onload = async function() {
            ...
            add_slider(Master, 'gk_LocalReverbByDistance_ReverbDecay', 0, 1.);
            add_slider(Master, 'gk_LocalReverbByDistance_Wet', 0, 1.);
            add_slider(Master, 'gk_SpatialReverb_ReverbDecay', 0, 4.);
            add_slider(Master, 'gk_Spatialize_SpeakerRigRadius', 2, 100.);
            add_slider(Master, 'gi_instrument_position_rate', 0, .5);
