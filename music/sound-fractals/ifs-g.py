...
class IfsToSoundfile(object):
	def __init__(self, iterations = 10, durationSeconds = 20.0, minimumHz = 27.5, maximumHz = 16000.0, width=8000, height=400):
	...
	def render(self, m):
		...
		for x in xrange(self.width):
			...
			for y in xrange(self.height):
				...
				self.soundfile.cosineGrain(t, self.grainSeconds, Hz, float(m[x, y]) * 0.001, 0.0, 0.0)
		print
...
if __name__ == '__main__':
	ifs = IfsToSoundfile()
	#                   a         b         c     d         e         f         w
	#ifs.addTransform(   1.0/3.0,  0.0,      0.0,  1.0/3.0,  1.0/3.0,  2.0/3.0,  1.0 )
	#ifs.addTransform(   0.0,      1.0/3.0,  1.0,  0.0,      2.0/3.0,  0.0,      1.0 )
	#ifs.addTransform(   0.0,     -1.0/3.0,  1.0,  0.0,      1.0/3.0,  0.0,      1.0 )
	ifs.addTransform(   1.0/3.0,  0.0,      0.0,  1.0/3.0,  1.0/3.0,  2.0/3.0,  1.0 )
	ifs.addTransform(   0.0,      1.0/3.0,  1.0,  0.0,      2.0/3.0,  0.0,      1.0 )
	ifs.addTransform(   0.0,     -1.0/3.0,  1.0,  0.0,      1.0/3.0,  0.0,      1.0 )
	ifs.iterate()
