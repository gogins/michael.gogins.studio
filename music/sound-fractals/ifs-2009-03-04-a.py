...
print
flameParametersFile = r'D:\utah\home\mkg\projects\music\01_workshop\ifs\renders.flame'
print 'Flame parameters file: %s' % flameParametersFile
flameParametersName = r'Apophysis-090305-97'
print 'Flame parameters name: %s' % flameParametersName
attractorColumns = 5000
print 'Columns in attractor:  %s' % attractorColumns
soundfileSeconds = 300
print 'Soundfile duration:    %s' % soundfileSeconds
...
stereoSoundfileName = title + '.stereo.wav'
print 'Stereo soundfile name: %s' % stereoSoundfileName
soundfilePlayer = r'D:\utah\opt\Audacity\audacity.exe'
print 'Soundfile player:      %s' % soundfilePlayer
linearFrequency = False
if linearFrequency:
    print 'Height is:             linear Hz'
else:
    print 'Height is:             log(Hz)'
print

class IfsToSoundfile(object):
    def __init__(self, 
    iterations = 10, 
    durationSeconds = soundfileSeconds, 
    minimumHz = 27.5 * 2.0, 
    maximumHz = 14000.0, 
    width=10000, 
    height=400,
    ...
    def render(self):
        ...
        for x in xrange(self.width):
            ...
            for y in xrange(self.height):
                # The image is upside down with respect to pitch (y origin at top), and must be inverted.
                f = self.height - y - 1
                # print f,y
                if self.heightIsFrequency:
                    Hz = self.lowestHz + (float(f) * self.hzSize)
                else:
                    key = self.lowestKey + (float(f) * self.keySize)
                    Hz = CsoundAC.Conversions_midiToHz(key)
                self.soundfile.cosineGrain(t, self.grainSeconds, Hz, float(self.target[x, y]) * 0.001, 0.0, 0.0, True, True)
...
<CsoundSynthesizer>
<CsOptions>
csound --messagelevel=99 -RWfo %s temp.orc temp.sco
</CsOptions>
<CsInstruments>
...
ipeak               filepeak    "%s"
                    ...
                    print       ipeak
                    xtratim     iduration
asignal             diskin2     "%s", 1.0
aleft1, aright1     reverbsc    asignal, a0,        0.93, 12000
aleft2, aright2     reverbsc    a0,      asignal,   0.91, 16000
                    outs        (aleft1 + aleft2) / 3, (aright1 + aright2) / 3
                    endin
...
i 1 1 1
        ...
        ''' % (stereoSoundfileName, soundfileName, soundfileName, soundfileName)
        print csd
        csound.setCSD(csd)
        csound.exportForPerformance()
        csound.perform()
    def play(self):
        os.spawnl(os.P_NOWAIT, soundfilePlayer, soundfilePlayer, r'"%s"' % (os.getcwd() + os.sep + stereoSoundfileName))

def parseFlame(flameFilename, flameName, ifs):
    print 'PARSING APOPHYSIS FRACTAL FLAME...'
    print
...
if __name__ == '__main__':
    ifs = IfsToSoundfile(width=attractorColumns, heightIsFrequency=linearFrequency)
    ...
    transformations = parseFlame(flameParametersFile, flameParametersName, ifs)
    ifs.iterate()
