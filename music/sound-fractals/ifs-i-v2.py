...
class IfsToSoundfile(object):
	...
	def initialize(self):
		...
		octaves = 8.0
		self.lowestKey = 24.0
		self.lowestHz = CsoundAC.Conversions.midiToHz(self.lowestKey)
		self.keyRange = 8.0 * 12.0
		self.highestKey = self.lowestKey + self.keyRange
		self.highestHz = CsoundAC.Conversions.midiToHz(self.highestKey)
	...
	def create(self):
		...
		self.soundfile.create(soundfileName, 48000, 1)
	...
	def apply(self, source, target):
		for x in range(self.width):
			for y in range(self.height):
				if source[x, y] > 0.0:
					for t in range(len(self.hutchinsonTransforms)):
	...
	def sizeIterate(self):
		...
		for self.iteration in range(self.sizeIterations):
			transform = self.spin()
	...
	def render(self, m):
		...
		for x in range(self.width):
			...
			for y in range(self.height):
				...
				Hz = CsoundAC.Conversions.midiToHz(key)
...
##	ifs.addTransform(   0.0000, -0.5000,  0.5000,  0.0000,  4.873085,  7.563492,  1.000 )
	ifs.addTransform(   0.5000,  0.0000,  0.0000,  0.5000,  4.663477, -7.563492,  1.000 )
	ifs.addTransform(   0.5000,  0.0000,  0.0000,  0.5000, -2.563477, -0.000003,  1.000 )
	ifs.addTransform(   0.5000,  0.0000,  0.0000,  0.5000,  2.436544, -0.000003,  2.000 )
	ifs.addTransform(   0.0000, -0.5000,  0.5500,  0.0000,  4.873085,  7.563492,  1.000 )
	ifs.iterate()
