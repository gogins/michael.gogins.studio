<CsoundSyntheizer>
<CsOptions>
-m195 -d -RWfoifs_explorer.wav --opcode-lib="/home/mkg/clang-opcodes/clang_opcodes.so"
</CsOptions>
<CsInstruments>
...
void rescale_time_and_duration(Score &score, double starting_time, double total_duration) {
    ...
    double minimum_start = score.front()[2];
    double maximum_end = minimum_start + score.front()[3];
    for (auto &note : score) {
        auto start = note[2];
        auto end = start + note[3];
    ...
    // move them to the target starting time.
    for (auto &note : score) {
        auto time = note[2];
        auto duration = note[3];
        time = time - starting_time;
        time = time * rescaling_factor;
...
std::string to_csound_score(const Score &score, bool twelve_tet=false) {
...
extern "C" int score_generator(CSOUND *csound) {
    ...
    rescale(scaling, score, 4, true, true,  5.,   10.0);
    rescale_time_and_duration(score, 2., 120.);
