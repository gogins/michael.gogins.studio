...
\item Next we get three bands of notes, but with varying dynamic ranges.
...
\begin{frame}{Hearing electroacoustically}
\begin{itemize}
\item In Audacity, zoom in on the short grain. Leave space before and after.
\item Change the spectrogram window to the smallest size (most wideband).
\item Observe that time is well-resolved, but \emph{all} frequencies are shown in the spectrogram, i.e. there is \emph{no} frequency resolution.
\item Change the spectrogram window to the largest size (most narrowband).
\item Observe that the frequency is now well-resolved, but the energy is smeared out \emph{well} before and after the grain.
\item This illustrates the time/frequency uncertainty that exists in all convolution and spectral processing.
\end{itemize}
\end{frame}
