...
R E D   L E A V E S   V E R S I O N   8 . 4 . 3 . 4
...
directly into the Csound orcheastra. The Csound instruments in this piece 
borrow and adapt instruments by Perry Cook, Ian McCurdy, Hans Mikelson, Steven 
Yi, and Lee Zakian, as well as myself. Spatialization is implemented using the 
IEM suite  of VST 2 plugins from IEM Graz, embedded in the Csound runtime 
using the vst4cs opcodes for Csound by Andres Cabrera and myself. The plugin 
Csound opcodes used in this piece are available on GitHub at 
...
NOTE: Must use spat3d, as only it supports moving sound sources.

Parameter   Purpose
0	        Early reflection recursion depth (0 is the sound source, 1 is the 
            first reflection etc.) for spat3d and spat3di. The number of 
            echoes for four walls (front, back, right, left) is: N = (2*R + 2) 
            * R. If all six walls are enabled: N = (((4*R + 6)*R + 8)*R) / 3.
1	        Late reflection recursion depth (used by spat3dt only). spat3dt 
            skips early reflections and renders echoes up to this level. If 
            early reflection depth is negative, spat3d and spat3di will output 
            zero, while spat3dt will start rendering from the sound source.
...
6 - 53	    wall parameters (w = 6: ceil, w = 14: floor, w = 22: front, w = 30: 
            back, w = 38: right, w = 46: left)
...
/*  used   distance  random  reflection  eq hz   eq level  eq q  eq mode            */ \
    1,     20,       0.05,   0.02,       4000.0, 0.6,      0.7,  2,      /* ceiling */ \
    1,     18,       0.03,   0.00,       3500.0, 0.5,      0.7,  2,      /* floor   */ \
    1,     19,       0.05,   0.03,       5000.0, 0.8,      0.7,  2,      /* front   */ \
    1,     20,       0.05,   0.03,       5000.0, 0.8,      0.7,  2,      /* back    */ \
    1,     15,       0.15,   0.07,       5000.0, 0.8,      0.7,  2,      /* right   */ \
    1,     20,       0.05,   0.08,       5000.0, 0.8,      0.7,  2       /* left    */
...
gi_Spatialize3D_verbose init 0
...
<html>
<head>
    <title>Red Leaves version 8.4.3.4</title>
...
if strcmp(gS_os, "macOS") == 0 then
gi_webserver webserver_create "/Users/michaelgogins/michael.gogins.studio/2022-NYCEMF/", 8080, 0
endif
